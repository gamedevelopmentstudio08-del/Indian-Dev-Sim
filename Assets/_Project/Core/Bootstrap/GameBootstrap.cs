...
public class GameBootstrap : MonoBehaviour
{
    ...
    private Camera gameplayMinimapCamera;
    private RawImage gameplayMinimapImage;
    private WeatherAnchorFollow weatherAnchorFollow;
    ...
    private void BuildScene()
    {
        ...
        CreateTrafficLights();
        CreateBus();
        CreateGameplayPickupStation();
        EnsureRouteGuidance();
        SetupGameplayMinimap();
        CreateMovingDemoVehicle();
        SetupCamera();
        CreateWeatherSystem();
        if (!HasManualMenuUi() && !IsPassengerScene())
        {
            CreateHud();
    ...
    private static bool IsPassengerScene()
    {
        string sceneName = SceneManager.GetActiveScene().name;
        return sceneName == "DropOffScene" || sceneName == "PickupScene";
    }
    ...
    private void CreateWeatherSystem()
    {
        ...
        rainObject.transform.position = bus != null
            ? bus.transform.position + new Vector3(0f, 16f, 12f)
        ...
        weatherAnchorFollow = rainObject.AddComponent<WeatherAnchorFollow>();
        weatherAnchorFollow.SetTarget(bus != null ? bus.transform : null, new Vector3(0f, 16f, 12f));
    ...
    private void ApplyWeatherInstant(WeatherSettings settings)
    {
        ...
        GameData.SetWeatherLabel(weatherLabel);
    ...
    private sealed class WeatherAnchorFollow : MonoBehaviour
    {
    ...
    private void CreateBus()
    {
        ...
        Vector3 spawnPosition = GameData.IsDropOffPhase ? GameData.SceneSpawnPosition : cityBusStartPosition;
        Quaternion spawnRotation = GameData.IsDropOffPhase ? GameData.SceneSpawnRotation : cityBusStartRotation;
        Transform spawnPoint = GameObject.Find(busSpawnPointName)?.transform;
        if (spawnPoint != null)
        {
        ...
        bus = spawner.SpawnBusFromResources(spawnPosition, spawnRotation, busSpawnPointName);
        if (bus == null)
        {
    ...
    private void CreateGameplayPickupStation()
    {
        Transform busTransform = bus != null ? bus.transform : null;
        Vector3 stationPosition = busTransform != null ? busTransform.position : cityBusStartPosition;
        GameObject root = GameObject.Find("BusStop");
        if (root == null)
        {
            root = new GameObject("BusStop");
        }

        root.transform.position = new Vector3(stationPosition.x, 0f, stationPosition.z);

        GameObject stand = GameObject.Find("BusStandPlatform");
        if (stand == null)
        {
        ...
        GameObject pickupZone = GameObject.Find("PickupZone");
        if (pickupZone == null)
        {
            pickupZone = GameObject.CreatePrimitive(PrimitiveType.Cube);
    ...
    private void SetupGameplayMinimap()
    {
        if (bus == null)
        {
            return;
        ...
        if (gameplayMinimapCamera == null)
        {
            ...
            gameplayMinimapCamera = cameraObject.GetComponent<Camera>();
            if (gameplayMinimapCamera == null)
            {
                gameplayMinimapCamera = cameraObject.AddComponent<Camera>();
            }

            gameplayMinimapCamera.orthographic = true;
            gameplayMinimapCamera.orthographicSize = 90f;
            gameplayMinimapCamera.clearFlags = CameraClearFlags.SolidColor;
            gameplayMinimapCamera.backgroundColor = new Color(0.08f, 0.10f, 0.12f, 1f);
            gameplayMinimapCamera.nearClipPlane = 0.1f;
            gameplayMinimapCamera.farClipPlane = 3000f;
            gameplayMinimapCamera.cullingMask = LayerMask.GetMask("Default", "Bus", "Traffic", "Environment", "Ground", "Minimap");
            gameplayMinimapCamera.targetTexture = new RenderTexture(512, 512, 16);

            MinimapFollow follow = cameraObject.GetComponent<MinimapFollow>();
            if (follow == null)
            {
                follow = cameraObject.AddComponent<MinimapFollow>();
            }

            follow.SetTarget(bus.transform);
            gameplayMinimapCamera.Render();
        }

        Canvas canvas = SceneUiHelper.EnsureOverlayCanvas("GameplayCanvas");
        gameplayMinimapImage = CreateOrGetMinimapImage(canvas.transform);
        if (gameplayMinimapCamera != null && gameplayMinimapImage != null)
        {
            gameplayMinimapImage.texture = gameplayMinimapCamera.targetTexture;
        }
    }
    ...
    private void SetupCamera()
    {
        ...
        cam.cullingMask = LayerMask.GetMask("Default", "Bus", "Traffic", "Environment", "Ground");
        ...
        follow.SetTarget(bus.transform);
        Transform driverSeat = bus.transform.Find("DriverSeat");
        if (driverSeat != null)
        {
            follow.driverSeat = driverSeat;
        }
